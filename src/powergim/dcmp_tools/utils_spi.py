...
def extract_all_opt_variable_values(model):
    ...
    for myvar in all_obj:
        ...
        if index_names is not None:
            ...
            # Assign a single index with specified name
            df.index = pd.Index(df.index, name='k')
...
class TimeseriesBuilder:
    ...
    def __init__(self, 
        ...
        if custom_ts_data_path is not None:
            ...
            self.TS_DATA_PATH = PGIM_INPUT_FILES_PATH.parents[2]/"timeseries_data" # standard directory structure: "timeseries_data", "inputs", "outputs" at same hierarchy level.
        os.makedirs(self.TS_DATA_PATH,exist_ok=True)

        
        self.pgim_ts = {'windoff'    : {'ts_path' : self.TS_DATA_PATH / Path('wind_power_offshore_' + self.data_year['res_ts'] + '.csv')},
    ...
    #------------------RES POWER TIME SERIES - RENEWABLE NNJA---------------------
    @staticmethod
    def get_ninja_res_ts(custom_path, ninja_token, data_locations, res_type, year = '2020'):
        # Timeseries based on indicative offshore/onshore locations corresponding to different nodes
        api_base = 'https://www.renewables.ninja/api/'

        if res_type in {'windon', 'windoff'}:
        
            turbine_model   = 'Gamesa G128 5000'
            wind_power_data = {key: None for key in list(data_locations.keys())}
            requests_counter = 0
        
            for iLocation in list(data_locations.keys()):
                requests_counter += 1
                
                s = requests.session()
                # Send token header with each request
                s.headers = {'Authorization': 'Token ' + ninja_token}
                url = api_base + 'data/wind'
                
                args = {
                        'lat': data_locations[iLocation]['lat'],
                        'lon': data_locations[iLocation]['lon'],
                        'date_from': year + '-01-01',
                        'date_to':   year + '-12-31',
                        'capacity': 1, # just to get profiles - capacity defined in pgim input csv (generators)
                        'height': 94,
    ...
    # =============================================================================
                ...
                wind_power_data[iLocation] = data.values.flatten().tolist()
                # metadata = parsed_response['metadata']
                # If necessary, wait, to stay within limit : max 50 per hour
                # i.e. should spend 72 sec per request. 72*50=1 hour
                # time.sleep(73)  # wait 73 sec to below 50 requests per hour
                time.sleep(1)
                if requests_counter % 48 == 0:  # 48 instead of 50 to be on the safe side.
                    # wait an hour
                    print(f"Waiting an hour - {time.asctime()}")
                    time.sleep(60 * 60)
        
        
            df_wind_power = pd.DataFrame.from_dict(wind_power_data)
            df_wind_power.set_index(data.index, inplace=True)       
            if res_type == 'windoff':
                fullPathName = 'wind_power_offshore_' + year + ".csv"
            elif res_type == 'windon':
                fullPathName = 'wind_power_onshore_' + year + ".csv"
           
            df_wind_power.to_csv(custom_path/fullPathName)
        
            return df_wind_power
        
        elif res_type == 'solar':
                    
            solar_power_data = {key: None for key in list(data_locations.keys())}
            requests_counter = 0
            ...
            for iLocation in list(data_locations.keys()):
                ...
                args = {
                        ...
                        'date_to':   year + '-12-31',
                        'dataset': 'merra2',
                        'capacity': 1.0, # just to get profiles - capacity defined in pgim input csv (generators)
                        'system_loss': 0.1,
                        'tracking': 0,
                        'tilt': 35,
                        'azim': 180,
                ...
                solar_power_data[iLocation] = data.values.flatten().tolist()
                # metadata = parsed_response['metadata']
                # If necessary, wait, to stay within limit : max 50 per hour
                # i.e. should spend 72 sec per request. 72*50=1 hour
                # time.sleep(73)  # wait 73 sec to below 50 requests per hour
            ...
            df_solar_power = pd.DataFrame.from_dict(solar_power_data)
            df_solar_power.set_index(data.index, inplace=True)       
            fullPathName = 'solar_power_' + year + ".csv"

            df_solar_power.to_csv(custom_path/fullPathName)

            return df_solar_power
        
        else:
            
            raise Exception('Please select "solar" / "windon" / "windoff" for res_type')

    @staticmethod
    def get_entsoe_ld_ts(save_data_path, entsoe_key, countries_codes, year = '2020'):
        client = EntsoePandasClient(api_key = entsoe_key)
    
        start_time = pd.Timestamp(year + '0101 00:00:00', tz='Europe/Oslo')
        end_time   = pd.Timestamp(str(int(year)+1) + '0101 00:00:00', tz='Europe/Oslo')

        countries_load_dic = {country_code : client.query_load(country_code, start=start_time,end=end_time) for country_code in countries_codes}
        countries_load_dic = {country_code : countries_load_dic[country_code].resample('1H').mean() for country_code in countries_codes}
    
        countries_load_df = pd.DataFrame({country_code : countries_load_dic[country_code].values[:,0] for country_code in countries_codes}, index = countries_load_dic['NO'].index )
        
        countries_load_scaled_df = (countries_load_df-countries_load_df.min())/(countries_load_df.max()-countries_load_df.min())    
        
        # mapping from [0,1] --> [0.5, 1.5] (to match the average demand in the consumers pgim input file)
        countries_load_scaled_avg_df = countries_load_scaled_df*(1.5-0.5) + 0.5
    
        countries_load_scaled_avg_df.to_csv(save_data_path/f"load_{year}.csv")
    
        return countries_load_df, countries_load_scaled_avg_df
            
            
    
    def get_timeseries(self, 
                       data_locations_offshore = {  'NO_SoennavindA': {'lat':57.51624269,'lon':7.391032075},
                       ...
                       data_locations_solar = { 'node_id_1': {'lat':57.51624269,'lon':7.391032075},
                                                'node_id_2': {'lat':56.81703138,'lon':4.872008852},
                                                },
                       data_locations_windon = {'node_id_1': {'lat':57.51624269,'lon':7.391032075},
                                                'node_id_2': {'lat':56.81703138,'lon':4.872008852},
                                                },
                       countries_codes=['UK', 'DK', 'DE', 'NL', 'NO', 'SE', 'BE']):
        
        """Check if the csv files for timeseries exists and if not, create them. 
            Taking extra care not to overcome the limit of 50 requests per hour."""
        
        if not self.pgim_ts['windoff']['ts_path'].is_file():
            print('Getting offshore wind generators timeseries...')
            TimeseriesBuilder.get_ninja_res_ts(self.TS_DATA_PATH, self.ninja_token, data_locations_offshore, res_type='windoff', year=self.data_year['res_ts'])
        
        if not self.pgim_ts['solar']['ts_path'].is_file():
            if len(data_locations_offshore) + len(data_locations_solar) > 50:
                print(f"Waiting an hour because max requests per hour (50) reached - {time.asctime()}")
                time.sleep(60 * 60)
                print('Getting onshore solar generators timeseries...')
                TimeseriesBuilder.get_ninja_res_ts(self.TS_DATA_PATH, self.ninja_token, data_locations_solar, res_type='solar', year=self.data_year['res_ts'])
            else:
                TimeseriesBuilder.get_ninja_res_ts(self.TS_DATA_PATH, self.ninja_token, data_locations_solar, res_type='solar', year=self.data_year['res_ts'])
        
        if not self.pgim_ts['windon']['ts_path'].is_file():
            if len(data_locations_offshore) + len(data_locations_solar) + len(data_locations_windon) > 50:
                print(f"Waiting an hour because max requests per hour (50) reached - {time.asctime()}")
                time.sleep(60 * 60)
                print('Getting onshore wind generators timeseries...')
                TimeseriesBuilder.get_ninja_res_ts(self.TS_DATA_PATH, self.ninja_token, data_locations_windon, res_type='windon', year=self.data_year['res_ts'])
            else:
                print('Getting onshore wind generators timeseries...')
                TimeseriesBuilder.get_ninja_res_ts(self.TS_DATA_PATH, self.ninja_token, data_locations_windon, res_type='windon', year=self.data_year['res_ts'])
        
        if not self.pgim_ts['consumption']['ts_path'].is_file():
            print('Getting ENTSOE load timeseries...')
            TimeseriesBuilder.get_entsoe_ld_ts(self.TS_DATA_PATH, self.entsoe_key, countries_codes, year=self.data_year['load_ts'])
    ...
    def create_csv(self):
        ...
        mask_existing_ts = [self.pgim_ts['windoff']['ts_path'].is_file(), self.pgim_ts['solar']['ts_path'].is_file(),
                            self.pgim_ts['windon']['ts_path'].is_file() , self.pgim_ts['consumption']['ts_path'].is_file()]
        ...
        if not self.TS_PATH.is_file():
            ...
            if (all(mask_existing_ts)):
                ...
                self.pgim_ts['consumption'].update({'data':pd.read_csv(self.pgim_ts['consumption']['ts_path'], index_col=0)})
                ...
                ts_data = pd.concat([self.pgim_ts['windoff']['data'],
                                     self.pgim_ts['consumption']['data'],
                                     self.pgim_ts['solar']['data'],
                                     self.pgim_ts['windon']['data']], axis=1
                ...
                raise Exception(f"{str([b for a, b in zip(map(lambda _x:not _x ,mask_existing_ts), list(self.pgim_ts.keys())) if a])[1:-1]} timeseries missing. \
...
class ReferenceModelCreator: # TODO: In future version attribute names can imitt prefixes/suffixes 'ref' or 'pgim' because they are implicetely implied. See corresponding 'CHECKME' notes.
    ...
    """
    def __init__(self,branches_file_name = "branches.csv",s_sample_size=5,sample_random_state=1,investment_periods=[2035,2040,2050],probabilities = {"scen0": 0.1, "scen1": 0.45, "scen2": 0.45},grid_case='baseline'):
    ...
    def create_reference_pgim_model(self,PATH_INPUT,PATH_OUTPUT,time_coupling=False,scenario_selection='default'):
        ...
        match self.grid_case:
            case 'baseline':
                ...
                self.ref_params = pgim.file_io.read_parameters(PATH_INPUT / "parameters.yaml") # CHECKME: `self.ref_params` ---> `self.params`
                ...
                if not time_coupling:
                    ...
                    self.ref_params['parameters']['load_flex_price_frac'] = {2035:0, 2040:0, 2050:0} # no price-sensitive load

                self.ref_grid_data = pgim.file_io.read_grid(investment_years=self.ref_params["parameters"]["investment_years"],
                ...
                if not time_coupling:
                    self.ref_grid_data.generator['pavg'] = 0     # For MH and decompostion methods: No energy constraint on hydro.
                    self.ref_grid_data.node['cost_scaling'] = 0  # For MH and decompostion methods: No node expansion cost.
                ...
                if scenario_selection == 'default':
                    ...
                    demand_scale = (260*self.probabilities['scen0']+220*self.probabilities['scen1']+340*self.probabilities['scen2'])/260
                    ...
                    for year in self.ref_params['parameters']['investment_years']:
                        ...
                        self.ref_grid_data.consumer.loc[m_demand, f"demand_{year}"] = demand_scale * self.ref_grid_data.consumer.loc[m_demand, f"demand_{year}"]
                    ...
                    raise ValueError(f"Allowed values for 'scenario_selection': {', '.join(map(str, ['default', 'EVP','A1', 'A2']))}")


                profiles_year = pgim.file_io.read_profiles(filename=PATH_INPUT / "time_series_year.csv")
                # -------------Sampling from whole year data
                profiles_sample = profiles_year.sample(n=self.s_sample_size, random_state=self.sample_random_state).reset_index()
                ...
                self.ref_pgim_model = pgim.SipModel(self.ref_grid_data, self.ref_params) # CHECKME: `self.ref_pgim_model` ---> `self.model`
            
            case 'star':
                # Read input grid (data)
                self.ref_grid_data , self.ref_params = create_case_star(investment_years=self.investment_periods,
                ...
                if scenario_selection == 'default':
                    ...
                    for iperiod in self.ref_params['parameters']['investment_years']:
                        self.ref_grid_data.generator.loc[self.ref_grid_data.generator['node'] == 'n1', ['capacity_' + str(iperiod)]] = self.probabilities['scen1']*0.5*_init_wind_capacity.loc[0,'capacity_' + str(iperiod)] + (self.probabilities['scen0']+self.probabilities['scen2'])*_init_wind_capacity.loc[0,'capacity_' + str(iperiod)]
                    ...
                    for iperiod in self.ref_params['parameters']['investment_years']:
                        self.ref_grid_data.consumer.loc[self.ref_grid_data.consumer['node'] == 'n3', ['demand_' + str(iperiod)]] = self.probabilities['scen2']*2*_init_load_capacity.loc[1,'demand_' + str(iperiod)] + (self.probabilities['scen0']+self.probabilities['scen1'])*_init_load_capacity.loc[1,'demand_' + str(iperiod)]
                    ...
                    for iperiod in self.ref_params['parameters']['investment_years']:
                        self.ref_grid_data.generator.loc[self.ref_grid_data.generator['node'] == 'n1', ['capacity_' + str(iperiod)]] = 0.5*_init_wind_capacity.loc[0,'capacity_' + str(iperiod)]
                    ...
                    for iperiod in self.ref_params['parameters']['investment_years']:
                        self.ref_grid_data.consumer.loc[self.ref_grid_data.consumer['node'] == 'n3', ['demand_' + str(iperiod)]] = 2*_init_load_capacity.loc[1,'demand_' + str(iperiod)]
                ...
                if self.randomize_profiles:
                    random.seed(0)
                    self.ref_grid_data.profiles['demand1'] = [random.random() for _ in range(self.s_sample_size)]
                    random.seed(1)
                    self.ref_grid_data.profiles['inflow_gentype0'] = [random.random() for _ in range(self.s_sample_size)]

                self.ref_pgim_model = pgim.SipModel(self.ref_grid_data, self.ref_params) # CHECKME: `self.ref_pgim_model` ---> `self.model`

            case _: return f"Invalid case specification. Choose from: {', '.join(map(str, ['baseline', 'star']))}."
...
class ParamsExtractor:
    ...
    def __init__(self,pgim_model=None):
        if pgim_model:
            ...
            for iSet in self.all_sets:
                if (iSet.name == 's_branch') or \
                    (iSet.name == 's_period') or \
                        (iSet.name == 's_time') or \
                            (iSet.name == 's_gen') or \
                                (iSet.name == 's_load'):
    ...
    def get_max_number_cables(self):
        ...
        _multi_index = pd.MultiIndex.from_tuples(product(self.my_sets['s_branch'], self.my_sets['s_period']),
                                                 names=['branch', 'period'])

        
        _temp_df_data = []        
        _specific_contraints = [cnstr for cnstr in self.all_contraints if cnstr.name.startswith(_specific_contraint_name)]
    ...
    def get_max_gen_power(self):
        ...
        _multi_index = pd.MultiIndex.from_tuples(product(self.my_sets['s_gen'], self.my_sets['s_period'], self.my_sets['s_time']),
    ...
    def get_max_load(self):
        ...
        _multi_index = pd.MultiIndex.from_tuples(product(self.my_sets['s_load'], self.my_sets['s_period'], self.my_sets['s_time']),
    ...
    def get_investment_cost_coefs(self):
        ...
        for key, inside_info in _temp_dict.items():
            _aggregated_dict[inside_info['label']] = _aggregated_dict.get(inside_info['label'], 0) + inside_info['value']

        # Specify the pattern to filter keys
        _pattern_branch_new_cables = 'v_branch_new_cables'     # we care only for the cost coefficients of variables: v_branch_new_cables
        _pattern_branch_new_capacity = 'v_branch_new_capacity' # we care only for the cost coefficients of variables: v_branch_new_capacity

        # Filter entries where the key starts with the specified pattern
        _filtered_dict_brach_new_cables = {key: value for key, value in _aggregated_dict.items() if key.startswith(_pattern_branch_new_cables)}
        _filtered_dict_brach_new_capacity = {key: value for key, value in _aggregated_dict.items() if key.startswith(_pattern_branch_new_capacity)}
        ...
        for br in self.my_sets['s_branch']:
            for pr in self.my_sets['s_period']:
                ...
                if _var_to_check in _filtered_dict_brach_new_cables:
                    _my_filtered_list_new_cables.append(_filtered_dict_brach_new_cables[_pattern_branch_new_cables+f'[{br},{pr}]'])
                else: 
        ...
        for br in self.my_sets['s_branch']:
            for pr in self.my_sets['s_period']:
                ...
                if _var_to_check in _filtered_dict_brach_new_capacity:
                    _my_filtered_list_new_capacity.append(_filtered_dict_brach_new_capacity[_pattern_branch_new_capacity+f'[{br},{pr}]'])
                else: 
    ...
    def get_operation_cost_coefs(self):
        ...
        costly_gens_condition = self.model.grid_data.generator['type'].isin(['gentype1','gas','hydro','oil','nonres_other','res_other','coal','nuclear'])
        ...
        if not isinstance(_set_nodes_with_costly_generators, list): _set_nodes_with_costly_generators = [_set_nodes_with_costly_generators]
                
        if isinstance(_set_nodes_with_costly_generators[0], int):
                
            _multi_index_gen = pd.MultiIndex.from_tuples(product(_set_nodes_with_costly_generators,
                                                            self.my_sets['s_period'], 
                                                            self.my_sets['s_time']),
                                                             names=['gen', 'period','time'])
        else:
            _multi_index_gen = pd.MultiIndex.from_tuples(product([list(ele) for ele in _set_nodes_with_costly_generators][0],
                                                            self.my_sets['s_period'], 
                                                            self.my_sets['s_time']),
                                                             names=['gen', 'period','time'])
                
        
        _multi_index_load_shed = pd.MultiIndex.from_tuples(product(self.my_sets['s_load'],
                                                        self.my_sets['s_period'], 
                                                        self.my_sets['s_time']),
                                                         names=['load', 'period','time'])
        ...
        _pattern_load_shed = 'v_load_shed'  # we care only for the cost coeeficients of variables: v_generation, v_load_shed
...
class CaseMapVisualizer:
    ...
    """
    def __init__(self, pgim_ref,rslt=None,data_locations_windoff=None,outFileName=None,outPath=None,visualize_rslt=False):
        self.all_periods = [2030]+pgim_ref.investment_periods
        self.grid_data = pgim_ref.ref_grid_data
        self.viz_rslt = visualize_rslt
        self.rslt = rslt
        if data_locations_windoff == None:
            self.data_locations_windoff  = {'NO_SoennavindA': {'lat':57.51624269,'lon':7.391032075},
                                            'NO_SoervestF'  : {'lat':56.81703138,'lon':4.872008852},
                                            'NO_SoervestB'  : {'lat':57.3480636,'lon':3.360271522},
                                            'NO_SoervestC'  : {'lat':57.03010167,'lon':3.885691333},
                                            'NO_VestavindF' : {'lat':59.275943,'lon':4.540937},
        ...
        if (outFileName == None) or (outPath == None):
    ...
    def create_html_map(self):
        ...
        boundaries = folium.WmsTileLayer(
                ...
                layers="MarineRegions:eez_boundaries", transparent=True,
                fmt="image/png",
        ...
        cumulative_generation_capacity = self.grid_data.generator.loc[(self.grid_data.generator['type'] == 'windoff')][capacity_column_names[1:]].cumsum(axis = 1)
        ...
        radius_weight_farm_symbol = {pgim_periods[0]:1, 
                                     pgim_periods[1]:1,
                                     pgim_periods[2]:1,
                                     pgim_periods[3]:1
                                     }
        ...
        feature_group_exist = folium.FeatureGroup(f'Radial WF connections existing at {pgim_periods[0]} (no further developements)')
        ...
        for ibranch, branch_row_data in self.grid_data.branch.iterrows():
            ...
            branch_locations = [(self.grid_data.node.at[branch_row_data['node_from'],"lat"],\
            ...
            if (branch_row_data[expand_column_names].sum() > 0):
                ...
                for _, iperiod in enumerate(pgim_periods):
                    if branch_row_data[expand_column_names[0]]>0 and iperiod == pgim_periods[1]:
                        folium.PolyLine(branch_locations, color="yellow", popup= f"variable_{branch_row_data['node_from']}-{branch_row_data['node_to']}_branch_index_{ibranch}", weight=1.5).add_to(feature_group_vars_period_1)
                        folium.PolyLine(branch_locations, color="black", dash_array='10', weight=1).add_to(feature_group_vars_period_1)
                    elif branch_row_data[expand_column_names[1]]>0 and iperiod == pgim_periods[2]:
                        folium.PolyLine(branch_locations, color="yellow", popup= f"variable_{branch_row_data['node_from']}-{branch_row_data['node_to']}_branch_index_{ibranch}", weight=1.5).add_to(feature_group_vars_period_2)
                        folium.PolyLine(branch_locations, color="black", dash_array='10', weight=1).add_to(feature_group_vars_period_2)
                    elif branch_row_data[expand_column_names[2]]>0 and iperiod == pgim_periods[3]:
                        folium.PolyLine(branch_locations, color="yellow", popup= f"variable_{branch_row_data['node_from']}-{branch_row_data['node_to']}_branch_index_{ibranch}", weight=1.5).add_to(feature_group_vars_period_3)
                        folium.PolyLine(branch_locations, color="black", dash_array='10', weight=1).add_to(feature_group_vars_period_3)
            ...
            elif (branch_row_data[capacity_column_names[0]] >= self.arbitarily_large_branch_capacity) and (branch_row_data['node_from'] != 'UK_scotland'):
                
                folium.PolyLine(branch_locations, color="pink", popup= f'onshore_grid_capacity_{branch_row_data[capacity_column_names[0]]}_MW', weight=2).add_to(feature_group_ons)
            ...
            # Identify specific "well-known" offshore interconnections.
                 ...
                 folium.PolyLine(branch_locations, color="green", popup= f"NorthSea Link_branch_index_{ibranch}_{branch_row_data[capacity_column_names[0]]}_MW", weight=2).add_to(feature_group_internat)
            elif branch_row_data['node_from'] == 'NO_kvinesdal' and branch_row_data['node_to'] == 'DE_L2':
                 folium.PolyLine(branch_locations, color="green", popup= f"NordLink__{ibranch}_{branch_row_data[capacity_column_names[0]]}_MW", weight=2).add_to(feature_group_internat)
            elif branch_row_data['node_from'] == 'NO_kvinesdal' and branch_row_data['node_to'] == 'NL_L4':
                 folium.PolyLine(branch_locations, color="green", popup= f"NorNed__{ibranch}_{branch_row_data[capacity_column_names[0]]}_MW", weight=2).add_to(feature_group_internat)
            else:
                if branch_row_data[capacity_column_names].sum() > 0:
                    ...
                    for _, iperiod in enumerate(pgim_periods):
                        if branch_row_data[capacity_column_names[0]]>0 and iperiod == pgim_periods[0]:
                            folium.PolyLine(branch_locations, color="black", popup= f"branch_{branch_row_data['node_from'][:2]}-{branch_row_data['node_to'][:2]}_branch_index_{ibranch}_total_capacity_{round(branch_row_data[capacity_column_names[0]])}_MW", weight=2, dash_array='10').add_to(feature_group_internat_period_0)
                        elif branch_row_data[capacity_column_names[1]]>0 and iperiod == pgim_periods[1]: 
                            folium.PolyLine(branch_locations, color="black", popup= f"branch_{branch_row_data['node_from'][:2]}-{branch_row_data['node_to'][:2]}_branch_index_{ibranch}_total_capacity_{round(branch_row_data[capacity_column_names[1]])}_MW", weight=2, dash_array='10').add_to(feature_group_internat_period_1)
                        elif branch_row_data[capacity_column_names[2]] > 0 and iperiod == pgim_periods[2]:
                            folium.PolyLine(branch_locations, color="black", popup= f"branch_{branch_row_data['node_from'][:2]}-{branch_row_data['node_to'][:2]}_branch_index_{ibranch}_total_capacity_{round(branch_row_data[capacity_column_names[1]]+branch_row_data[capacity_column_names[2]])}_MW", weight=2, dash_array='10').add_to(feature_group_internat_period_2)
                        elif branch_row_data[capacity_column_names[3]] > 0 and iperiod == pgim_periods[3]:
                            folium.PolyLine(branch_locations, color="black", popup= f"branch_{branch_row_data['node_from'][:2]}-{branch_row_data['node_to'][:2]}_branch_index_{ibranch}_total_capacity_{round(branch_row_data[capacity_column_names[1]]+branch_row_data[capacity_column_names[2]]+branch_row_data[capacity_column_names[3]])}_MW", weight=2, dash_array='10').add_to(feature_group_internat_period_3)
            ...
            if self.viz_rslt == True:
                for n_strgc in _strategic_nodes:
                    x_cbl = self.rslt['z_new_lines'].xs(n_strgc,level=1).loc[self.rslt['z_new_lines'].xs(n_strgc,level=1).values>0.01]
                    x_cpt = self.rslt['z_new_capacity'].xs(n_strgc,level=1).loc[self.rslt['z_new_capacity'].xs(n_strgc,level=1).values>0.01]

                    if len(_strategic_nodes)<=3:

                        if (ibranch in x_cbl.index.get_level_values('branch')) and (ibranch in x_cpt.index.get_level_values('branch')):
                            cables = round(x_cbl.loc[ibranch])
                            capacity = round(x_cpt.loc[ibranch])
                            if n_strgc == 0:
                                folium.PolyLine(branch_locations, color="blue", popup= f"investment_{branch_row_data['node_from']}-{branch_row_data['node_to']}_new_lines_{cables}_total_capacity_{capacity}", weight=3).add_to(feature_group_rslt_period_1)
                            elif n_strgc==1:
                                folium.PolyLine(branch_locations, color="blue", popup= f"investment_{branch_row_data['node_from']}-{branch_row_data['node_to']}_new_lines_{cables}_total_capacity_{capacity}", weight=3).add_to(feature_group_rslt_period_2)
                            else:
                                folium.PolyLine(branch_locations, color="blue", popup= f"investment_{branch_row_data['node_from']}-{branch_row_data['node_to']}_new_lines_{cables}_total_capacity_{capacity}", weight=3).add_to(feature_group_rslt_period_3)
                    else:
                        if (ibranch in x_cbl.index.get_level_values('branch')) and (ibranch in x_cpt.index.get_level_values('branch')):
                            cables = round(x_cbl.loc[ibranch])
                            capacity = round(x_cpt.loc[ibranch])
                            if n_strgc == 0:
                                ...
                                folium.PolyLine(branch_locations, color="blue", popup= f"investment_{branch_row_data['node_from']}-{branch_row_data['node_to']}_new_lines_{cables}_total_capacity_{capacity}", weight=3).add_to(feature_group_rslt_period_2_scen_1)
                            elif n_strgc==4:
                                folium.PolyLine(branch_locations, color="blue", popup= f"investment_{branch_row_data['node_from']}-{branch_row_data['node_to']}_new_lines_{cables}_total_capacity_{capacity}", weight=3).add_to(feature_group_rslt_period_3_scen_1)
                            elif n_strgc==2:
                                folium.PolyLine(branch_locations, color="blue", popup= f"investment_{branch_row_data['node_from']}-{branch_row_data['node_to']}_new_lines_{cables}_total_capacity_{capacity}", weight=3).add_to(feature_group_rslt_period_2_scen_0)
                            elif n_strgc==5:
                                folium.PolyLine(branch_locations, color="blue", popup= f"investment_{branch_row_data['node_from']}-{branch_row_data['node_to']}_new_lines_{cables}_total_capacity_{capacity}", weight=3).add_to(feature_group_rslt_period_3_scen_0)
                            elif n_strgc==3:
                                folium.PolyLine(branch_locations, color="blue", popup= f"investment_{branch_row_data['node_from']}-{branch_row_data['node_to']}_new_lines_{cables}_total_capacity_{capacity}", weight=3).add_to(feature_group_rslt_period_2_scen_2)
                            elif n_strgc==6:
                                folium.PolyLine(branch_locations, color="blue", popup= f"investment_{branch_row_data['node_from']}-{branch_row_data['node_to']}_new_lines_{cables}_total_capacity_{capacity}", weight=3).add_to(feature_group_rslt_period_3_scen_2)



        test_keys = ['UK', 'DK', 'DE', 'NL', 'NO', 'SE', 'BE']
        colors    = ['red','blue','green','orange','purple','cadetblue','darkred','pink','darkgreen','lightgreen','darkblue',
                     'beige','lightred','lightblue','darkpurple','gray','lightgray','black']
            ...
            if gen_row_data['comment'] == 'offshorewind':
                  ...
                  if (gen_row_data[capacity_column_names[1:]].sum() - gen_row_data[capacity_column_names[0]]) > 0: 
                      ...
                      for idx, iperiod in enumerate(pgim_periods):
                          ...
                          df_cum_sum = gen_row_data[capacity_column_names].cumsum() - gen_row_data[capacity_column_names[0]]
                         
                          radius_farm_symbol_interp = np.interp(df_cum_sum[capacity_column_names[idx]], 
                                                                capacity_levels, radius_farm_symbol)
                          ...
                          if gen_row_data[capacity_column_names[0]] > 0 and iperiod == pgim_periods[0]:  
                              folium.CircleMarker([gen_row_data['lat'], gen_row_data['lon']],
                                                  radius=radius_farm_symbol_interp, fill=True, fill_opacity=0.4,
                                                  ...
                                                  color=colorMap[self.grid_data.node.at[gen_row_data['node'], 'area']]).add_to(feature_group_gens_period_0)
                        ...
                        # Identify farm capacity developements in the first investment period [2035].
                          elif (gen_row_data[capacity_column_names[1]] - gen_row_data[capacity_column_names[0]]) > 0 and iperiod == pgim_periods[1]:

                                  folium.CircleMarker([gen_row_data['lat'], gen_row_data['lon']],
                                                      radius=radius_farm_symbol_interp, fill=False, weight=radius_weight_farm_symbol[iperiod],
                                                      popup= f"WF_{gen_row_data['node']}_({round(gen_row_data[capacity_column_names[1:]].cumsum()[capacity_column_names[-1]])}_MW)",
                                                      color=colorMap[self.grid_data.node.at[gen_row_data['node'], 'area']]).add_to(feature_group_gens_period_1)
                        ...
                        # Identify farm capacity developements in the second investment period [2040].
                                  ...
                                  folium.CircleMarker([gen_row_data['lat'], gen_row_data['lon']],
                                                          radius=radius_farm_symbol_interp, fill=False, weight=radius_weight_farm_symbol[iperiod],
                                                          popup= f"WF_{gen_row_data['node']}_({round(gen_row_data[capacity_column_names[1:]].cumsum()[capacity_column_names[-1]])}_MW)",
                                                          color=colorMap[self.grid_data.node.at[gen_row_data['node'], 'area']]).add_to(feature_group_gens_period_2)
                        ...
                        # Identify farm capacity developements in the thrid investment period [2050].
                                  ...
                                  folium.CircleMarker([gen_row_data['lat'], gen_row_data['lon']],
                                                          radius=radius_farm_symbol_interp, fill=False, weight=radius_weight_farm_symbol[iperiod],
                                                          popup= f"WF_{gen_row_data['node']}_({round(gen_row_data[capacity_column_names[1:]].cumsum()[capacity_column_names[-1]])}_MW)",
                                                          color=colorMap[self.grid_data.node.at[gen_row_data['node'], 'area']]).add_to(feature_group_gens_period_3)

                  else:
                      
                      folium.Marker([gen_row_data['lat'], gen_row_data['lon']],
                                    popup= f"WF_{gen_row_data['node']}_({round(gen_row_data[capacity_column_names[0]])}_MW)",
                                        icon=folium.DivIcon(html=f"""
                                                                    <div><svg height ="10" width="10">
                                                                       ...
                                                                       </svg></div>""")).add_to(feature_group_exist)
                      
                      if (self.grid_data.node.index == gen_row_data['node']).any():
                            
                            branch_locations = [(gen_row_data['lat'],\
                                                  gen_row_data['lon']),
                  ...
                  if (gen_row_data['node'].startswith(tuple(prefixes))) or (gen_row_data['node'].endswith(('east','west','main','scotland')))\
                  ...
                  for node_id, node_row_data in self.grid_data.node.iterrows():
                      ...
                      if gen_row_data['node'] == node_id and \
                        (gen_row_data[capacity_column_names[1:]].sum() - gen_row_data[capacity_column_names[0]]) > 0 and \
                            (not gen_row_data['node'].startswith(tuple(prefixes))) and \
                                (not gen_row_data['node'].endswith(('east','west','hub'))):
        ...
        for node_id, node_row_data in self.grid_data.node.iterrows():
            ...
            if node_id in self.grid_data.consumer["node"].values.tolist():
                folium.Marker([node_row_data['lat'], node_row_data['lon']], radius=1000, popup= "load_" + node_id, icon=folium.Icon(icon="glyphicon-home",prefix='glyphicon')).add_to(myMap)
            else:
                folium.CircleMarker([node_row_data['lat'], node_row_data['lon']],
                                    radius=self.radius_farm_symbol_nominal/6, fill=True,fill_opacity=1, fill_color='black',
                                    popup= f"node_id_{node_id}",
                                    color='black').add_to(feature_group_nodes)
            ...
            if (node_id.endswith('_main')) or (node_id.startswith('NO_') and node_row_data['comment'] == 'NO_split'):
                ...
                if node_id.startswith('NO_'):
                    gens_cluster = folium.plugins.MarkerCluster(name=f"generators_cluster_{node_id}").add_to(feature_group_generators)
                else:
                    gens_cluster = folium.plugins.MarkerCluster(name=f"generators_cluster_{node_row_data['area']}").add_to(feature_group_generators)
                ...
                if (node_id.startswith('NO_') and node_row_data['comment'] == 'NO_split'):
                    aggregated_node_generation_capacity = df_generators[capacity_column_names[0]].loc[(df_generators['comment'] == 'mainland')].loc[df_generators['node'].str.startswith('NO_')].sum() 
                    radius_scaling_farm_symbol = 10
                else:
                    aggregated_node_generation_capacity = df_generators[capacity_column_names[0]].loc[(df_generators['comment'] == 'mainland')].loc[(df_generators['node'] == node_id)].sum()
                ...
                for igen, gen_row_data in df_generators.loc[df_generators['comment'] == 'mainland'].loc[df_generators['node'] == node_id].iterrows():
                    
                    if gen_row_data['type'] != 'windoff':
                        match gen_row_data['type']:
                            case 'gas': genTypeColor = 'cadetblue'
                            case 'coal': genTypeColor = 'gray'
                            case 'hydro': genTypeColor = 'blue'
                            case 'nonres_other': genTypeColor = 'purple'
                            case 'oil': genTypeColor = 'black'
                            case 'res_other': genTypeColor = 'lightgreen'
                            case 'solar': genTypeColor = 'orange'
                            case 'windon': genTypeColor = 'green'
                            case 'nuclear': genTypeColor = 'red'
                            case 'flex': genTypeColor = 'pink'
                            case _: print(f"{gen_row_data['type']} generator type is missing")
                        
                    genTypeRadiusprcnt = gen_row_data[capacity_column_names[0]] / aggregated_node_generation_capacity + 0.01         
                    
                    gen_marker = folium.CircleMarker([gen_row_data['lat'], gen_row_data['lon']], 
                                                     ...
                                                     fill=True,  fill_opacity=0.8, color=genTypeColor)
        ...
        condition = lambda ele, code : ele.startswith(code) or (ele.startswith('GB_') and code == 'UK')
        ...
        for izone in list(self.data_locations_windoff.keys()):
            ...
            folium.Marker([self.data_locations_windoff[izone]["lat"], self.data_locations_windoff[izone]["lon"]],
                            radius=1000, popup= "wind_data_" + izone,
                            icon=folium.Icon(color=colorMap[test_keys[colorIdx[0]]], icon="glyphicon-folder-open",prefix='glyphicon')).add_to(feature_group_windoff)
