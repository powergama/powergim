...
def fix_first_stage_vars(model,sol):
    ...
    investment_vars  = [var for var in all_variables if (var.name.startswith('z_new_lines') or var.name.startswith('z_new_capacity') or var.name.startswith('z_capacity_total'))]
...
def calc_vss(case_settings,RP,path_in,path_out):
    ...
    #-----------------------------CREATING EVP MODEL-----------------------------
    pgim_evp = ReferenceModelCreator(grid_case=GRID_CASE,
    ...
    optimal_solution_evp_dict = {"mh_rslt": 
                                    {
    ...
    #-----------------------------CREATING EEVP MODEL-----------------------------
    pgim_eevp = ReferenceModelCreator(grid_case=GRID_CASE,
                                      branches_file_name=BRANCHES_FILE_NAME,
                                      s_sample_size=N_SAMPLES,
                                      probabilities=PROB
                                      )
...
if __name__ == "__main__":
    ...
    vss, eev = calc_vss(run_settings,RP_OBJ_VAL,PATH_INPUT,PATH_OUTPUT)
