...
from .decomposition import MultiHorizonPgim,BendersDecomp
from .utils_spi import extract_all_opt_variable_values,TimeseriesBuilder,ReferenceModelCreator,CaseMapVisualizer
...
def pgim_ref_init(input_options:dict):
    ...
    if not input_options['IS_STOCHASTIC']: input_options['DO_CALC_VSS_EVPI'] = False
    if input_options['BENDERS_SINGLE_SUB']: input_options['BENDERS_SINGLE_CUT']=True
    if (input_options['GRID_CASE'] == 'star') or (input_options['MH_USE_BIN_EXPANS']): input_options['DO_VIZ'] = False

    #---------------------GETTING/CREATING REQUIRED TIMESERIES CSV FILE-----------------------------
    if input_options['GRID_CASE'] == 'baseline':
        TimeseriesBuilder(input_options['PATH_INPUT']).create_csv() # checking/creating existence of 1 year-long timeseries CSV for pgim.

    #-----------------------------CREATING BASELINE MODEL-----------------------------
    pgim_ref = ReferenceModelCreator(grid_case=input_options['GRID_CASE'],
                                                        branches_file_name=input_options['BRANCHES_FILE_NAME'],
                                                        s_sample_size=input_options['N_SAMPLES'],
                                                        probabilities=PROB
                                                        )
    pgim_ref.create_reference_pgim_model(input_options['PATH_INPUT'],input_options['PATH_OUTPUT'],scenario_selection='default')
...
def do_mh(input_options:dict,
    ...
    if input_options['DO_VIZ'] and input_options['GRID_CASE'] == 'baseline':
        case_visualizer = CaseMapVisualizer(pgim_ref,outFileName='case_map_interactive.html',outPath=input_options['PATH_OUTPUT'])
    ...
    if not input_options['IS_STOCHASTIC']:
        ...
        if input_options['GRID_CASE'] == 'star':
            ...
            def my_mpisppy_scenario_creator(scenario_name):
                ...
                pgim_scen_model = pgim_model.scenario_creator(scenario_name, probability=input_options['PROB'][scenario_name]) # B) Use scenario_creator method to build a scenario instance model
            ...
            def my_mpisppy_scenario_creator(scenario_name):
                ...
                pgim_scen_model = pgim_model.scenario_creator(scenario_name, probability=input_options['PROB'][scenario_name])
...
def do_bd(input_options:dict,
    ...
    for iter_Bd in range(0, my_BD_setup.MAX_BD_ITER + 1):
        ...
        if iter_Bd == 0:
            ...
            if input_options['BENDERS_SINGLE_CUT']:
                ...
                master_Bd_model, master_Bd_solver = my_BD_setup.create_master_problem(USE_MULTI_CUTS=not input_options['BENDERS_SINGLE_CUT'])
    ...
    if input_options['DO_CNVRG_PLOT']:
        ...
        fig2save = input_options['PATH_OUTPUT'] / f"benders_convergence_plot_case_{input_options['GRID_CASE']}_stochastic_{input_options['IS_STOCHASTIC']}_samples_{input_options['N_SAMPLES']}_multiCut_{not input_options['BENDERS_SINGLE_CUT']}_fixed_cap_{input_options['MH_USE_FIXED_CAP']}_{timestamp}.pdf"
        fig.savefig(fig2save, format='pdf')
    else:
        print(f"\n\ninput_options['DO_CNVRG_PLOT'] = {input_options['DO_CNVRG_PLOT']}) ---> Convergence plot was NOT saved.\n\n")

    return solution_Master_Bd, master_Bd_model, my_BD_setup, elapsed_time_delta, temp_a_sum, UB_Bd, LB_Bd, iter_Bd, iter_Bd_2_plot, x_current_Bd

#%%
def results_log(input_options:dict,
                pgim_ref:ReferenceModelCreator,
    ...
    if not mh_ref.is_stochastic:
        ...
        # EXPECTED OPEX
        pgim_opex = round(  input_options['PROB']['scen0']*optimal_solution_pgim[0]["scen0.v_operating_cost"].sum() + \
...
def get_first_stage_values(get_first_stage_decision_pgim:Callable,
                           optimal_solution_pgim:dict,
                           mh_ref:MultiHorizonPgim,
...
#%%
def save_result_files(input_options:dict,
                      timestamp:str,
    ...
    results_file_out_path = input_options['PATH_OUTPUT']/f"run_Benders_validation_{timestamp}.pickle" # write a PICKLE file for results.
...
#%%
def calc_stochastic_metrics(input_options:dict,
                            timestamp:str,
                            report_to_save:dict,
                            report_file_out_path:str
                            ):
    ...
    if input_options['DO_CALC_VSS_EVPI']:
        ...
        current_vss, current_eev = calc_vss(run_settings,RP_OBJ_VAL,input_options['PATH_INPUT'],input_options['PATH_OUTPUT'])

        print(f'\n EEV = {round(current_eev)}')
        print(f'\n VSS = {round(current_vss)}')

        current_evpi, current_ws = calc_evpi(run_settings,RP_OBJ_VAL,input_options['PATH_INPUT'],input_options['PATH_OUTPUT'])
...
#%%
def visualize_results(input_options:dict,
                      timestamp:str
                      ):
    ...
    if input_options['DO_VIZ']:
        if input_options['GRID_CASE'] == 'baseline':
            create_interactive_maps_results(run_settings,run_rslt,input_options['PATH_INPUT'],input_options['PATH_OUTPUT'])
            create_geopandas_figures_results(timestamp,run_settings,run_rslt,input_options['PATH_INPUT'],input_options['PATH_OUTPUT'])

        print("\n---------------Graphics have been geenrated successfully.")

#%%
def run_case_bd(now_time:datetime, 
                input_options:dict = {'PATH_INPUT': Path(__file__).parents[3] / "examples"/ "inputs"/"CASE_BASELINE",
    ...
    solution_Master_Bd, master_Bd_model, my_BD_setup, \
    elapsed_time_delta, temp_a_sum, UB_Bd, LB_Bd, \
    iter_Bd, iter_Bd_2_plot, x_current_Bd \
    = do_bd(input_options,mh_ref,timestamp)

    pgim_obj_val, pgim_opex, \
    mh_obj_val, bd_obj_val, bd_opex, \
    get_first_stage_decision_pgim \
    = results_log(input_options,pgim_ref,optimal_solution_pgim,my_pgim_stochastic_model_ef,mh_ref,master_Bd_model,temp_a_sum)
    
    x_pgim, x_mh, x_bd = get_first_stage_values(get_first_stage_decision_pgim,optimal_solution_pgim,mh_ref,optimal_solution_multi_hor,x_current_Bd)
    
    report_to_save, report_file_out_path = save_result_files(input_options,
                                                             timestamp,
                                                             formatted_now_time,
                                                             elapsed_time_delta,
                                                             my_BD_setup,
