...
INPUT_OPTIONS = {
    "PATH_INPUT": Path(__file__).parents[1] / "examples" / "inputs" / "CASE_BASELINE",
...
@pytest.mark.skipif(
    not pyo.SolverFactory(INPUT_OPTIONS["LP_SOLVER"]).available(),
    reason=f"Skipping test because solver {INPUT_OPTIONS['LP_SOLVER']}is not available.",
)
def test_run_case_bd(capsys, stochastic_case):
    ...
    pgim_obj_val, mh_obj_val, bd_obj_val, pgim_opex, bd_opex = run_case_bd(
        datetime.now(), test_input_options
    )  # Call the function
